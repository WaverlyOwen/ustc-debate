...
| `\begin{sourcelist}` | `\source{#}{论据}{出处}{原文引句}{核实日期}{状态}{用在}{查证方向}`，空的参数留 `{}` | 论据出处清单；`check_evidence.py` 只认这个宏。数据的「局限」只写在证据卡的 `\sub{局限}` 里，出处清单不重复 |
...
- 需要转义的字符：`&` `%` `$` `#` `_` 写成 `\&` `\%` `\$` `\#` `\_`；网址用 `\url{…}`，网址里的中文（如汉典词条）要写成百分号编码，且百分号写成 `\%`（例：`\url{https://zdic.net/hans/\%E7\%88\%B1}`），否则 PDF 里是乱码方块或编译报错
...
- 压页时注意：`\section` 和 `\begin{stage}` 前面各保留了几行的最小空间（needspace），一个只有两三行的小节会整节被推到下一页。速查超一页时先看最后一页是不是只有一个小节的尾巴，是的话删两行或合并小节，不要以为内容超了很多
- 速查里的出处只写作者 + 年份（或机构 + 年份），卷期页码留给备赛文档的出处清单
